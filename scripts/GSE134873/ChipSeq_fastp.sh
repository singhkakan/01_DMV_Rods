...
mkdir "$GROUP_SCRATCH/GSE134873/ChipSeq/fastq_trimmed"
fastp_out_dir="$GROUP_SCRATCH/GSE134873/ChipSeq/fastq_trimmed"
fasta="$GROUP_HOME/XCD_BSSeq/NOVOGENE_2025/adapter.fa"
fastq_dir="$GROUP_SCRATCH/GSE134873/ChipSeq/fastq"
...
do    
    outname="${j%%.*}"
    fastp -w 9 -y -M 30 -3 -5 -l 28 --adapter_fasta="$fasta" -i $fastq_dir/"$j" -o $fastp_out_dir/"$outname".fq.gz -h $fastp_out_dir/"$outname"
done
