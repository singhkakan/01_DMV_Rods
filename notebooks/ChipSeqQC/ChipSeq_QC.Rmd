...
do
    ...
    --bdg \
    -t $i \
    -c SRR4252949.sam.bam SRR4252950.sam.bam \
    -n "$outname"_narrowPeak \
    --outdir $Output 2> $Output/log/"$outname"_narrowPeak-macs2.log
...
Samples <- read.table("/home/groups/ximenac/XCD_BSSeq/R_analysis/ChipSeqQC/ChipSeq_metadata.txt", sep=',', header=TRUE)
#rownames(Samples) <- paste0(Samples$Tissue, ":",Samples$Factor, "-", Samples$Replicate)
Samples = Samples[-c(16),] #It does not work if you include the actual control sample
#View(Samples)
```


```{r Loading blacklist regions, message=FALSE, warning=FALSE, include=FALSE, paged.print=FALSE}
...
RelCC is low for #14 H3K9me3, and this will require further investigation. 
...
### RiP
RiP stands for reads in peaks. This is again an indicator of good enrichment. ChIPs around 5% or higher usually indicate successful enrichment. Rip scores are good/acceptable for all samples except H3K4me1.


## Plotting Reads in Peak
```{r fig.align='center', fig.height=6.5, fig.width=9, message=FALSE, warning=FALSE, paged.print=FALSE}
plotRap(chipObj, facetBy=c("Factor"))
```
...
```
Nearly all replicates cluster together, except for the two H3K27Ac replicates from the P21 (Aldiri's) data set. These will need to be evaluated further. These samples have fairly close read depths, but there is a two fold difference in the number of peaks called in one replicate.  

H3K9me3 clusters with the replicates of H3K27me3.

H3K4Me1, H3K27me3 and H3K27Ac cluster very close together in both the P21 and isolated Rods data set, whereas H3K4me3 from the two data sets cluster separately.
...
plotPrincomp(chipObj, attributes = "Factor", label="Tissue", dotSize=0.7, labelSize=0.5, vColors=col, alpha=0.7, text(pos = 4))
```
...
Replicate 1 of H3K27Ac clusters with the two H3K27me3, H3K9me3 and H3K4me1. However, this may be due to the low quality of peak calls of these samples, as all of the have a RiP score of less than 10% or a low RellCC score.
...
```{r echo=FALSE, fig.height=16.5, fig.width=7, message=FALSE, warning=FALSE, paged.print=FALSE}
...
```{r fig.height=12, fig.width=7.5, message=FALSE, warning=FALSE, paged.print=FALSE}
plotSSD(chipObj)
```

To assess the contribution of artefact signal to global distribution of signal pile-up we measure SSD before and after exclusion of signal from known blacklisted regions. This explains some of the discrepancies we see in the H3K9me3 sample. SSD signal for H3K9me3 is greatly affected by blacklisted regions.
...
H3K27me3_3 <- QCsample(chipObj,19)
H3K27me3_4 <- QCsample(chipObj,20)
...
H3K27Ac_3 <- QCsample(chipObj,17)
H3K27Ac_4 <- QCsample(chipObj,18)
...
H3K4Me3_3 <- QCsample(chipObj,23)
H3K4Me3_4 <- QCsample(chipObj,24)
...
H3K4Me1_2 <- QCsample(chipObj,21)
H3K4Me1_3 <- QCsample(chipObj,22)
