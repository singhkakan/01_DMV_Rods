...
```{r Read in DMV Bed files into a list}
base_path <- "/home/groups/ximenac/01_DMV_Rods/notebooks/"
...
cell_type <- c("HSC", "E8.5", "Ret E11.5", "Ret E12.5", "Ret E14.5", "Ret E17.5", "Ret P3", "Ret P21","Rod 3m", "Cone 2m","FC 1w", "FC 2w",  "NeuN+")
...
```{r convert genome types bed list to gr}
...
```{r Create Overlap df for DMV and gene regions}
...
for (i in 1:length(cell_type)) {
  ...
  DMV = as.data.frame(DMV_Beds_gr[[i]])[queryHits(overlaps_cellType_genomeRegion[[i]][[j]]), ],
  Region = as.data.frame(mmu_genome_regions_gr[[j]])[subjectHits(overlaps_cellType_genomeRegion[[i]][[j]]), ]
)
  }
}
```
...
for (i in 1:length(cell_type)) {
  ...
  DMV = as.data.frame(uDMV_Beds_gr[[i]])[queryHits(uDMV_overlaps_cellType_genomeRegion[[i]][[j]]), ],
  Region = as.data.frame(mmu_genome_regions_gr[[j]])[subjectHits(uDMV_overlaps_cellType_genomeRegion[[i]][[j]]), ]
)
  }
}
```
...
for (i in 1:length(cell_type)) {
  for (j in 1:length(genome_type)) {
    file_name <- paste0(cell_type[i], "_DMV_", genome_type[j], ".csv")
    write.csv(overlap_df_DMV[[i]][[j]], file=file_name, quote=FALSE, row.names = FALSE)
  }
}
...
for (i in 1:length(cell_type)) {
  for (j in 1:length(genome_type)) {
    file_name <- paste0(cell_type[i], "_uDMV_", genome_type[j], ".csv")
    write.csv(overlap_df_uDMV[[i]][[j]], file=file_name, quote=FALSE, row.names = FALSE)
  }
}

```

#Genic Intergenic Figure 
#Create Master DF for all cell-type DMVs by genome region types genic intergenic
```{r For All DMVs - Generate DF for Genic Intergenic Figure, message=FALSE, warning=FALSE}

#Stack DMV df by genic regions and intergenic regions for a given cell type

genic_intergenic <- list()

for (i in 1:length(cell_type)) {
  genic_intergenic[[i]] <- rbind(unique(overlap_df_DMV[[i]][[1]][,c(1:6,14)]), unique(overlap_df_DMV[[i]][[3]][,c(1:6,12)]))
  names(genic_intergenic)[i] <- cell_type[i]
}

#Create unique DMV ID
#Annotate DMVs as genic only, intergenic only, or both
for (i in 1:length(cell_type)) {
  genic_intergenic[[i]]$DMV_ID <- paste0(genic_intergenic[[i]]$DMV.seqnames, ":", 
                                             genic_intergenic[[i]]$DMV.start, "-",
                                             genic_intergenic[[i]]$DMV.end)
  genic_intergenic[[i]]$genic_intergenic <- ifelse(isUnique(genic_intergenic[[i]]$DMV_ID), genic_intergenic[[i]]$Region.Region, "Both")
  genic_intergenic[[i]]$genic_intergenic <- ifelse(genic_intergenic[[i]]$genic_intergenic=="Known_genes", "Genic", genic_intergenic[[i]]$genic_intergenic)
  cut <- genic_intergenic[[i]][which(genic_intergenic[[i]]$genic_intergenic=="Both"),]
  genic_intergenic[[i]] <- rbind(genic_intergenic[[i]][which(isUnique(genic_intergenic[[i]]$DMV_ID)),],
                          cut[which(!duplicated(cut$DMV_ID)),])
}


```

#Summarize and Modify the Genic Intergenic Dataframe
```{r}
#Stack DMVs of all cell types and create a new column indicating type of the DMV
#Use dplyr gather

library(tidyverse)
combined_genic_intergenic_df <- map_df(genic_intergenic, ~ .x, .id = "id")
combined_genic_intergenic_df$genic_intergenic <- as.factor(combined_genic_intergenic_df$genic_intergenic)

combined_genic_intergenic_summary <- map_df(genic_intergenic, ~ {
  .x %>%
    count(genic_intergenic) %>%
    mutate(genic_intergenic = as.character(genic_intergenic))
}, .id="id")

#Adjust the factor levels so the order in the figure is not alphabetical
combined_genic_intergenic_summary <- combined_genic_intergenic_summary %>%
  mutate(id=factor(id)) %>%
  mutate(id=fct_relevel(id, c(
                              "HSC",
                              "E8.5", 
                              "Ret E11.5","Ret E12.5","Ret E14.5", 
                              "Ret E17.5", 
                              "Ret P3",
                              "Ret P21",
                              "Rod 3m",
...
#Figure Genic Intergenic 
```{r Figure Genic Intergenic }
setwd("/home/groups/ximenac/01_DMV_Rods/notebooks/")
...
color=c(rep(plotColors[1], each = 1),
        rep(plotColors[2:3], each = 1),
        rep(plotColors[3:7], each = 1),
        rep(plotColors[8], each = 2), 
        rep(plotColors[9], each = 2), 
        rep(plotColors[10], each = 1),
        rep(plotColors[1], each = 1)
        )

tiff("final_outs/Rsc05_021326_Genic_Intergenic_Barplot.tiff", units = "in", height = 4.25, width = 6.75, res = 300)
ggplot(combined_genic_intergenic_summary, aes(fill=genic_intergenic, y=n)) + 
  geom_bar(aes(x=id), stat="identity") +
  ylab("Number of DMVs") +
  scale_color_manual(values=color) +
  scale_fill_viridis(discrete = T) +
  geom_text(aes(x=id, label = n), color="white", hjust=-0.05,  size=4.5, family="Calibri") +
  theme_minimal() + 
  theme(axis.text.y = element_text(hjust=1, vjust=1.15, size=13, family="Calibri", color = color),
